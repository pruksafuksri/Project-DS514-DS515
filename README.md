...
ขั้นตอนการสร้างโมเดล
ระบุตัวแปรต้น (Features) และตัวแปรตาม (Target)
แปลงข้อมูล โดยการทำ Log Transformation กับข้อมูลบางตัวแปรที่มีความเบ้สูง ให้มีการกระจายตัวสมมาตรขึ้น
แบ่งข้อมูลเป็นชุด Train/Test โดยควบคุมสัดส่วน (Stratify) ของข้อมูลDiabetesไว้ตามเดิม
Scaling ปรับสเกลข้อมูลให้อยู่ในมาตรฐานเดียวกันด้วย StandardScaler
GridSearchCV หาค่าKที่ดีที่สุด และหารูปแบบการถ่วงน้ำหนัก(weights) ทดลองทั้งแบบ uniform (ให้น้ำหนักข้อมูลเท่ากัน) และ distance (ให้ข้อมูลที่อยู่ใกล้มีน้ำหนักมากกว่า) เพื่อให้โมเดลมีความแม่นยำ(ค่าAccuracyสูงสุด)
Cross-Validation (CV) เช็คคะแนน 5 รอบ ดูความเสถียร
สร้างและเทรนโมเดล K-Nearest Neighbors Model โดยใช้ค่า best parameters
Evaluation ประเมินผลประสิทธิภาพโมเดล ดูค่า Accuracy, Classification Report และ Confusion Matrix
